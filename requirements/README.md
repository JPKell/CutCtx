...
They do **not** define what a consumer installs. `pip install cutctx` resolves the compatible
...
`ci.lock` is short by suite standards, and deliberately: `cutctx`'s only runtime dependency is
`baseaicore`, whose own dependency count is zero (gold standards §1.1, where CutCtx's non-suite
budget is `0`). Everything else in the file is the development toolchain.

`hypothesis` is in `ci.lock` and is **new to the suite's toolchain with this repository**. It is a
development dependency: CutCtx's invariants are stated over *arbitrary* transcripts, so they are
tested that way (spec §18). It does not touch the runtime budget, and nothing under `src/` imports
it — a test asserts the whole import allowlist.
...
them — passing `--no-index` yourself does not, it fails to resolve. `release.lock` is
byte-identical to LoadLedger's, WeightsDB's, MirrorWall's and ModelRack's below the header, which
is the check that the chain really is reproducible rather than merely pinned.
...
`src/cutctx`. A path-based source reports 0 % against a non-editable install — the tests all pass,
